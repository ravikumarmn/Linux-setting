...
__conda_setup="$('/home/sigmoid/miniconda3/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/home/sigmoid/miniconda3/etc/profile.d/conda.sh" ]; then
        . "/home/sigmoid/miniconda3/etc/profile.d/conda.sh"
    else
        export PATH="/home/sigmoid/miniconda3/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<
...
####################################################################################### CUSTOM-CODE #######################################################################################
# create dirctory & change current directory to new directory
mkcd(){
	mkdir -p  "$1" && cd "$1"
}
